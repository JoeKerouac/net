...
final class CipherBox {
     ...
     */
    private CipherBox(ProtocolVersion protocolVersion, BulkCipher bulkCipher, SecretKey key,
     ...
     */
    static CipherBox newCipherBox(ProtocolVersion version, BulkCipher cipher, SecretKey key,
                                  IvParameterSpec iv, SecureRandom random,
                                  boolean encrypt) throws NoSuchAlgorithmException {
        if (!cipher.allowed) {
        ...
        try {
            ...
            if (cipherType == AEAD_CIPHER) {
                try {
                    ...
                    // unlikely to happen
                    throw new RuntimeException("Cipher error in AEAD mode in JCE provider "
                ...
                if (newLen != len) {
                    ...
                    throw new RuntimeException("Cipher buffering error " + "in JCE provider "
                                               + cipher.getProvider().getName());
                }
                return newLen;
            }
    ...
    int encrypt(ByteBuffer bb, int outLimit) {
        ...
        if (cipherType == AEAD_CIPHER) {
            try {
                ...
                // unlikely to happen
                throw new RuntimeException(
                    "Cipher error in AEAD mode in JCE provider " + cipher.getProvider().getName(),
                    ibse);
            }
        } else {
            int newLen;
        ...
        try {
            ...
            if (cipherType == BLOCK_CIPHER) {
                ...
                if (protocolVersion.v >= ProtocolVersion.TLS11.v) {
                    if (newLen < blockSize) {
                        throw new BadPaddingException("The length after " + "padding removal ("
                                                      + newLen + ") should be larger " + "than <"
                                                      + blockSize + "> since explicit IV used");
                    }
                }
            }
            return newLen;
        ...
        if (protocolVersion.v >= ProtocolVersion.TLS10.v) {
            ...
            if (padLen > blockSize) {
                throw new BadPaddingException(
                    "Padding length (" + padLen + ") of SSLv3 message should not be bigger "
    ...
    private static int removePadding(ByteBuffer bb, int tagLen, int blockSize,
        ...
        // The padding data should be filled with the padding length value.
        int[] results = checkPadding((ByteBuffer) bb.duplicate().position(offset + newLen),
            (byte) (padLen & 0xFF));
        if (protocolVersion.v >= ProtocolVersion.TLS10.v) {
            if (results[0] != 0) { // padding data has invalid bytes
                throw new BadPaddingException("Invalid TLS padding data");
    ...
    int applyExplicitNonce(Authenticator authenticator, byte contentType,
        ...
        switch (cipherType) {
            ...
            case AEAD_CIPHER:
                ...
                byte[] aad = authenticator.acquireAuthenticationBytes(contentType,
                    bb.remaining() - recordIvSize - tagSize);
     ...
     * @return the explicit nonce size of the cipher.
     */
    int applyExplicitNonce(Authenticator authenticator, byte contentType, byte[] buf, int offset,
