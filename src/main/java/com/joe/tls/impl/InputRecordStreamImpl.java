...
public class InputRecordStreamImpl implements InputRecordStream {
    ...
    public InputRecordStreamImpl(InputStream netStream, HandshakeHash handshakeHash, TlsVersion version) {
    ...
    public Record read() throws IOException {
        ...
        if (version == null) {
            throw new RuntimeException(
                String.format("不支持的version，major version: %d, minor version: %d", header[1], header[2]));
        }
        ...
        switch (contentType) {
            ...
            case APPLICATION_DATA:
                return new Record(ContentType.APPLICATION_DATA, version, new ApplicationMsg(ByteBuffer.wrap(content)));
            default:
    ...
    private byte[] decrypt(byte contentType, byte[] data) {
        ...
        if (cipherDesc.getCipherType() == CipherSuite.CipherType.AEAD) {
            ...
            if (Arrays.equals(mac,
                Arrays.copyOfRange(decryptResult, decryptResult.length - macLen, decryptResult.length))) {
