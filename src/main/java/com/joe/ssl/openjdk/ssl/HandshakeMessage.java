...
public abstract class HandshakeMessage {
    ...
    final void write(HandshakeOutStream s) throws IOException {
        ...
        if (len >= Record.OVERFLOW_OF_INT24) {
            throw new SSLException(
                "Handshake message too big" + ", type = " + messageType() + ", len = " + len);
        }
        s.write(messageType());
        s.putInt24(len);
        send(s);
    ...
    static final class ClientHello extends HandshakeMessage {
        ...
        void addRenegotiationInfoExtension(byte[] clientVerifyData) {
            HelloExtension renegotiationInfo = new RenegotiationInfoExtension(clientVerifyData,
                new byte[0]);
        ...
        CertificateMsg(HandshakeInStream input) throws IOException {
            ...
            while (chainLen > 0) {
                ...
                try {
                    ...
                    throw (SSLProtocolException) new SSLProtocolException(e.getMessage())
                        .initCause(e);
                }
            }
    ...
    static final class RSA_ServerKeyExchange extends ServerKeyExchange {
         ...
         */
        RSA_ServerKeyExchange(HandshakeInStream input) throws IOException,
    ...
    static final class DH_ServerKeyExchange extends ServerKeyExchange {
         ...
         */
        DH_ServerKeyExchange(DHCrypt obj, PrivateKey key, byte clntNonce[], byte svrNonce[],
                             SecureRandom sr, SignatureAndHashAlgorithm signAlgorithm,
        ...
        DH_ServerKeyExchange(HandshakeInStream input,
            ...
            KeyUtil.validate(new DHPublicKeySpec(new BigInteger(1, dh_Ys), new BigInteger(1, dh_p),
         ...
         */
        DH_ServerKeyExchange(HandshakeInStream input, PublicKey publicKey, byte clntNonce[],
                             byte svrNonce[], int messageSize,
            ...
            if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                ...
                preferableSignatureAlgorithm = SignatureAndHashAlgorithm.valueOf(hash, signature,
                    0);
    ...
    static final class ECDH_ServerKeyExchange extends ServerKeyExchange {
        ...
        ECDH_ServerKeyExchange(ECDHCrypt obj, PrivateKey privateKey, byte[] clntNonce,
         ...
         */
        ECDH_ServerKeyExchange(HandshakeInStream input, PublicKey signingKey, byte[] clntNonce,
                               byte[] svrNonce,
                               Collection<SignatureAndHashAlgorithm> localSupportedSignAlgs,
                               ProtocolVersion protocolVersion) throws IOException,
                                                                GeneralSecurityException {
    ...
    static final class CertificateRequest extends HandshakeMessage {
        ...
        private final static byte[]                   TYPES_NO_ECC         = { cct_rsa_sign,
                                                                               cct_dss_sign };
        private final static byte[]                   TYPES_ECC            = { cct_rsa_sign,
                                                                               cct_dss_sign,
                                                                               cct_ecdsa_sign };
        ...
        CertificateRequest(X509Certificate ca[], CipherSuite.KeyExchange keyExchange,
        ...
        CertificateRequest(HandshakeInStream input,
            ...
            if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                ...
                while (remains > 1) { // needs at least two bytes
                    ...
                    SignatureAndHashAlgorithm algorithm = SignatureAndHashAlgorithm.valueOf(hash,
                        signature, ++sequence);
            ...
            while (len >= 3) {
                HandshakeMessage.DistinguishedName dn = new HandshakeMessage.DistinguishedName(
                    input);
                v.add(dn);
                len -= dn.length();
            }
    ...
    static final class CertificateVerify extends HandshakeMessage {
         ...
         */
        CertificateVerify(ProtocolVersion protocolVersion, HandshakeHash handshakeHash,
                          PrivateKey privateKey, SecretKey masterSecret, SecureRandom sr,
                          SignatureAndHashAlgorithm signAlgorithm) throws GeneralSecurityException {
        ...
        CertificateVerify(HandshakeInStream input,
            ...
            if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                ...
                preferableSignatureAlgorithm = SignatureAndHashAlgorithm.valueOf(hashAlg, signAlg,
                    0);

                // Is it a local supported signature algorithm?
                if (!localSupportedSignAlgs.contains(preferableSignatureAlgorithm)) {
                    throw new SSLHandshakeException(
                        "Unsupported SignatureAndHashAlgorithm in " + "CertificateVerify message: "
                                                    + preferableSignatureAlgorithm);
                }
            }
         ...
         */
        boolean verify(ProtocolVersion protocolVersion, HandshakeHash handshakeHash,
                       PublicKey publicKey,
         ...
         */
        private static void updateSignature(Signature sig, ProtocolVersion protocolVersion,
                                            HandshakeHash handshakeHash, String algorithm,
                                            SecretKey masterKey) throws SignatureException {
         ...
         */
        private static void updateDigest(MessageDigest md, byte[] pad1, byte[] pad2,
        ...
        void print(PrintStream s) throws IOException {
            ...
            if (debug != null && Debug.isOn("verbose")) {
                if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                    s.println(
                        "Signature Algorithm " + preferableSignatureAlgorithm.getAlgorithmName());
                }
            }
        }
    }
    ...
    static final class Finished extends HandshakeMessage {
         ...
         */
        Finished(ProtocolVersion protocolVersion, HandshakeHash handshakeHash, int sender,
        ...
        private byte[] getFinished(HandshakeHash handshakeHash, int sender, SecretKey masterKey) {
            ...
            if (protocolVersion.v >= ProtocolVersion.TLS10.v) {
                ...
                try {
                     ...
                     */
                    TlsPrfParameterSpec spec = new TlsPrfParameterSpec(masterKey, tlsLabel, seed,
                        12, prfHashAlg, prfHashLength, prfBlockSize);
