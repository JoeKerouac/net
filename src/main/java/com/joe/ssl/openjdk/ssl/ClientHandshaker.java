...
final class ClientHandshaker extends Handshaker {
    ...
    private final static boolean                enableSNIExtension          = Debug
        .getBooleanProperty("jsse.enableSNIExtension", true);
     ...
     */
    private final static boolean                allowUnsafeServerCertChange = Debug
        .getBooleanProperty("jdk.tls.allowUnsafeServerCertChange", false);

    private List<SNIServerName>                 requestedServerNames        = Collections
        .<SNIServerName> emptyList();
    ...
    ClientHandshaker(SSLSocketImpl socket, SSLContextImpl context, ProtocolList enabledProtocols,
        ...
        super(socket, context, enabledProtocols, true, true, activeProtocolVersion,
            isInitialHandshake, secureRenegotiation, clientVerifyData, serverVerifyData);
    }
    ...
    ClientHandshaker(SSLEngineImpl engine, SSLContextImpl context, ProtocolList enabledProtocols,
        ...
        super(engine, context, enabledProtocols, true, true, activeProtocolVersion,
            isInitialHandshake, secureRenegotiation, clientVerifyData, serverVerifyData);
    }
    ...
    void processMessage(byte type, int messageLen) throws IOException {
        ...
        switch (type) {
            case HandshakeMessage.ht_hello_request:
                HandshakeMessage.HelloRequest helloRequest = new HandshakeMessage.HelloRequest(
                    input);
            ...
            case HandshakeMessage.ht_server_hello:
                HandshakeMessage.ServerHello serverHello = new HandshakeMessage.ServerHello(input,
                    messageLen);
            ...
            case HandshakeMessage.ht_certificate:
                ...
                HandshakeMessage.CertificateMsg certificateMsg = new HandshakeMessage.CertificateMsg(
                    input);
            ...
            case HandshakeMessage.ht_server_key_exchange:
                ...
                switch (keyExchange) {
                    case K_RSA_EXPORT:
                        ...
                        try {
                            HandshakeMessage.RSA_ServerKeyExchange rsaSrvKeyExchange = new HandshakeMessage.RSA_ServerKeyExchange(
                                input);
                    ...
                    case K_DH_ANON:
                        try {
                            HandshakeMessage.DH_ServerKeyExchange dhSrvKeyExchange = new HandshakeMessage.DH_ServerKeyExchange(
                    ...
                    case K_DHE_RSA:
                        try {
                            HandshakeMessage.DH_ServerKeyExchange dhSrvKeyExchange = new HandshakeMessage.DH_ServerKeyExchange(
                                input, serverKey, clnt_random.random_bytes, svr_random.random_bytes,
                                messageLen, getLocalSupportedSignAlgs(), protocolVersion);
                            handshakeState.update(dhSrvKeyExchange, resumingSession);
                            this.serverKeyExchange(dhSrvKeyExchange);
                        } catch (GeneralSecurityException e) {
                            throwSSLException("Server key", e);
                    ...
                    case K_ECDH_ANON:
                        try {
                            HandshakeMessage.ECDH_ServerKeyExchange ecdhSrvKeyExchange = new HandshakeMessage.ECDH_ServerKeyExchange(
                                input, serverKey, clnt_random.random_bytes, svr_random.random_bytes,
            ...
            case HandshakeMessage.ht_certificate_request:
                ...
                if ((keyExchange == K_DH_ANON) || (keyExchange == K_ECDH_ANON)) {
                    throw new SSLHandshakeException(
                        "Client authentication requested for " + "anonymous cipher suite.");
                } else if (keyExchange == K_KRB5 || keyExchange == K_KRB5_EXPORT) {
                    throw new SSLHandshakeException(
                        "Client certificate requested for " + "kerberos cipher suite.");
                }
                ...
                if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                    Collection<SignatureAndHashAlgorithm> peerSignAlgs = certRequest
                        .getSignAlgorithms();
                    ...
                    Collection<SignatureAndHashAlgorithm> supportedPeerSignAlgs = SignatureAndHashAlgorithm
                        .getSupportedAlgorithms(algorithmConstraints, peerSignAlgs);
            ...
            case HandshakeMessage.ht_finished:
                HandshakeMessage.Finished serverFinished = new HandshakeMessage.Finished(
                    protocolVersion, input, cipherSuite);
    ...
    private void serverHello(HandshakeMessage.ServerHello mesg) throws IOException {
        ...
        if (!isNegotiable(mesgVersion)) {
            throw new SSLHandshakeException("Server chose " + mesgVersion
        ...
        if (serverHelloRI != null) {
            if (isInitialHandshake) {
                ...
                System.arraycopy(serverVerifyData, 0, verifyData, clientVerifyData.length,
        ...
        if (mesg.compression_method != 0) {
            fatalSE(Alerts.alert_illegal_parameter,
                "compression type not supported, " + mesg.compression_method);
            // NOTREACHED
        }
        ...
        if (session != null) {
            ...
            if (session.getSessionId().equals(mesg.sessionId)) {
                ...
                if (sessionSuite.keyExchange == K_KRB5
                    ...
                    try {
                        subject = AccessController
                            .doPrivileged(new PrivilegedExceptionAction<Subject>() {
                                @Override
                    ...
                    if (subject != null) {
                        ...
                        if (!principals.contains(localPrincipal)) {
                            throw new SSLProtocolException(
                                "Server resumed" + " session with wrong subject identity");
                        } else {
                            if (debug != null && Debug.isOn("session"))
        ...
        ExtendedMasterSecretExtension extendedMasterSecretExt = (ExtendedMasterSecretExtension) mesg.extensions
            .get(ExtensionType.EXT_EXTENDED_MASTER_SECRET);
        if (extendedMasterSecretExt != null) {
            // Is it the expected server extension?
            if (!useExtendedMasterSecret || !(mesgVersion.v >= ProtocolVersion.TLS10.v)
                || !requestedToUseEMS) {
                fatalSE(Alerts.alert_unsupported_extension,
                    "Server sent the extended_master_secret " + "extension improperly");
            }
            ...
            if (resumingSession && (session != null) && !session.getUseExtendedMasterSecret()) {
                fatalSE(Alerts.alert_unsupported_extension,
            ...
            if (resumingSession && (session != null)) {
                if (session.getUseExtendedMasterSecret()) {
                    ...
                    fatalSE(Alerts.alert_handshake_failure,
                        "Missing Extended Master Secret extension " + "on session resumption");
        ...
        session = new SSLSessionImpl(protocolVersion, cipherSuite, getLocalSupportedSignAlgs(),
            mesg.sessionId, getHostSE(), getPortSE(), (extendedMasterSecretExt != null),
     ...
     */
    private void serverKeyExchange(HandshakeMessage.RSA_ServerKeyExchange mesg) throws IOException,
        ...
        if (!algorithmConstraints.permits(EnumSet.of(CryptoPrimitive.KEY_AGREEMENT),
            ephemeralServerKey)) {

            throw new SSLHandshakeException(
                "RSA ServerKeyExchange " + "does not comply to algorithm constraints");
        }
    }
    ...
    private void serverKeyExchange(HandshakeMessage.ECDH_ServerKeyExchange mesg) throws IOException {
        ...
        if (!algorithmConstraints.permits(EnumSet.of(CryptoPrimitive.KEY_AGREEMENT),
            ...
            throw new SSLHandshakeException(
                "ECDH ServerKeyExchange " + "does not comply to algorithm constraints");
        }
    }
    ...
    private void serverHelloDone(HandshakeMessage.ServerHelloDone mesg) throws IOException {
        ...
        if (certRequest != null) {
            ...
            if (keytypesTmpSize != 0) {
                ...
                if (conn != null) {
                    ...
                    alias = km.chooseEngineClientAlias(keytypes, certRequest.getAuthorities(),
                        engine);
                }
            }
            ...
            if (m1 == null) {
                ...
                if (debug != null && Debug.isOn("handshake")) {
                    System.out.println("Warning: no suitable certificate found - "
        ...
        switch (keyExchange) {
            ...
            case K_KRB5_EXPORT:
                ...
                if (sniHostname != null) {
                    ...
                    try {
                        kerberosMsg = new KerberosClientKeyExchange(sniHostname, getAccSE(),
                        ...
                        if (debug != null && Debug.isOn("handshake")) {
                            System.out.println(
                                "Warning, cannot use Server Name Indication: " + e.getMessage());
                        }
                    }
                }
                ...
                if (kerberosMsg == null) {
                    ...
                    if (hostname == null) {
                        throw new IOException(
                            "Hostname is required" + " to use Kerberos cipher suites");
                    }
                    kerberosMsg = new KerberosClientKeyExchange(hostname, getAccSE(),
                        protocolVersion, sslContext.getSecureRandom());
                }
        ...
        switch (keyExchange) {
            ...
            case K_KRB5:
            case K_KRB5_EXPORT:
                byte[] secretBytes = ((KerberosClientKeyExchange) m2)
            ...
            try {
                ...
                if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                    preferableSignatureAlgorithm = SignatureAndHashAlgorithm.getPreferableAlgorithm(
                        getPeerSupportedSignAlgs(), signingKey.getAlgorithm(), signingKey);

                    if (preferableSignatureAlgorithm == null) {
                        throw new SSLHandshakeException("No supported signature algorithm");
                    }

                    String hashAlg = SignatureAndHashAlgorithm
                        .getHashAlgorithmName(preferableSignatureAlgorithm);
                ...
                m3 = new HandshakeMessage.CertificateVerify(protocolVersion, handshakeHash,
                    signingKey, session.getMasterSecret(), sslContext.getSecureRandom(),
    ...
    HandshakeMessage getKickstartMessage() throws SSLException {
        ...
        //
        session = ((SSLSessionContextImpl) sslContext.engineGetClientSessionContext())
        ...
        if (session != null) {
            ...
            if (session != null && identityAlg != null) {
                ...
                String sessionIdentityAlg = session.getEndpointIdentificationAlgorithm();
                if (!Objects.equals(identityAlg, sessionIdentityAlg)) {

                    if (debug != null && Debug.isOn("session")) {
                        System.out.println("%% can't resume, endpoint id"
                                           + " algorithm does not match, requested: " + identityAlg
                                           + ", cached: " + sessionIdentityAlg);
                    }
                    session = null;
                }
            }
            ...
            if (session != null) {
                if (debug != null) {
                    if (Debug.isOn("handshake") || Debug.isOn("session")) {
                        System.out.println(
                            "%% Try resuming " + session + " from port " + getLocalPortSE());
                    }
                }
        ...
        HandshakeMessage.ClientHello clientHelloMessage = new HandshakeMessage.ClientHello(
            sslContext.getSecureRandom(), maxProtocolVersion, sessionId, cipherSuites);

        // add elliptic curves and point format extensions
        if (cipherSuites.containsEC()) {
            EllipticCurvesExtension ece = EllipticCurvesExtension
                .createExtension(algorithmConstraints);
            if (ece != null) {
    ...
    private void serverCertificate(HandshakeMessage.CertificateMsg mesg) throws IOException {
        ...
        if ((reservedServerCerts != null) && !session.getUseExtendedMasterSecret()) {
        ...
        try {
            ...
            if (tm instanceof X509ExtendedTrustManager) {
                if (conn != null) {
                    ((X509ExtendedTrustManager) tm).checkServerTrusted(peerCerts.clone(),
                        keyExchangeString, conn);
                } else {
                    ((X509ExtendedTrustManager) tm).checkServerTrusted(peerCerts.clone(),
                        keyExchangeString, engine);
                }
            } else {
    ...
    private static boolean isIdentityEquivalent(X509Certificate thisCert,
        ...
        if ((thisSubjectAltNames != null) && (prevSubjectAltNames != null)) {
            ...
            Collection<String> thisSubAltIPAddrs = getSubjectAltNames(thisSubjectAltNames,
                ALTNAME_IP);
            Collection<String> prevSubAltIPAddrs = getSubjectAltNames(prevSubjectAltNames,
                ALTNAME_IP);
            ...
            Collection<String> thisSubAltDnsNames = getSubjectAltNames(thisSubjectAltNames,
                ALTNAME_DNS);
            Collection<String> prevSubAltDnsNames = getSubjectAltNames(prevSubjectAltNames,
                ALTNAME_DNS);
        ...
        if (!thisSubject.getName().isEmpty() && !prevSubject.getName().isEmpty()
            && thisSubject.equals(prevSubject) && thisIssuer.equals(prevIssuer)) {
            return true;
        }
