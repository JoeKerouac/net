...
public class CipherBox {
    ...
    public CipherBox(SecureRandom secureRandom, CipherSuite.CipherDesc cipherDesc, byte[] cipherKey,
                     byte[] iv, boolean encrypt) {
     ...
     * @param authenticator 写出（加密）令牌
     * @param contentType contentType
     * @param fragmentLength 要加密的数据的长度
     * @return 随机数
     */
    public byte[] createExplicitNonce(Authenticator authenticator, byte contentType,
                                      int fragmentLength) {
        byte[] nonce;
     ...
     * @param authenticator 读取（解密）令牌
     * @param contentType contentType
     * @param data record层消息体，主要是AEAD模式为了从里边读取iv使用
    ...
    public int applyExplicitNonce(Authenticator authenticator, byte contentType, byte[] data) {
        if (cipherDesc.getCipherType() == CipherSuite.CipherType.BLOCK) {
            ...
            byte[] aad = authenticator.acquireAuthenticationBytes(contentType,
                data.length - recordIvSize - tagSize);
            cipherSpi.updateAAD(aad);
            return recordIvSize;
        } else {
            throw new RuntimeException("不支持的加密模式：" + cipherDesc);
     ...
     * @param buf 要填充的数据
     * @param offset 真实数据起始位置
     * @param len 数据长度
     * @param blockSize cipher的blockSize
     * @return 填充后的数据
     */
     ...
     * 检查padding是否正确
     * @param buf 缓冲数据
     * @param offset pad起始位置，注意：这里是pad的起始位置
     * @param len pad的长度
     * @param pad pad
